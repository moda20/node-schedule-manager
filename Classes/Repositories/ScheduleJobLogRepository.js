...
class ScheduleJobLogRepository {
  ...
  static async getLogStats(jobId) {
    try {
      ...
      let sql = `SELECT job_id as id, AVG(start_time - end_time) as avgTime, MAX(start_time) as latestStart, MAX(end_time) as LatestEnds from schedule_job_log group by job_id ${jobId ? `where job_id IN ?`: ''}`;
      sqlData.push(`(${jobIds.map(e=>"\'"+e+"\'").join(',')})`);
      if(sql === '') {
        return {success:false, err: 'get log failed'};
      }
