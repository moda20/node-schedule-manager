...
```javascript
let result = await ScheduleJobManager.newJob(
  "jobName",
  "*/1 * * * *",
  { jobParam: "value" },
  "/SampleJobConsumer.js",
  true,
  "ACTIVE",
);
 ...
 *   {String} err - Error message
 */
let result = await ScheduleJobManager.jobRegistration(jobId, {
  singular: true,
});
```
...
```javascript
let jobLogsResult = await ScheduleJobManager.getJobLog({
  order: "asc",
  offset: 0,
  limit: 10,
});
